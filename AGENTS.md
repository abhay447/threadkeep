...
- Local-only WhatsApp **Export chat** ZIP viewer. Not a WhatsApp client.
...
- Never write index/config/cache next to source. Use `getDataDir()` / the OS user-data directory. Ignore `WA_ARCHIVE_DATA_DIR` if it resolves inside the repo.
...
- UI: WhatsApp-like colors, Indic font stack (Nirmala UI, Kalinga). Conversation list is virtualized; measure rows from `top: 0`.
...
- *Media omitted from this export* = WhatsApp’s without-media export.
- Incremental import skips unchanged ZIPs. After parser changes that affect existing chats, wipe the data dir and reindex.

## Tests

- `npm test` (Vitest). API tests may set `WA_ALLOW_PATH=1` and a temp `WA_ARCHIVE_DATA_DIR` **outside** the repo (`os.tmpdir()`).
- Do not point tests at a real WhatsApp backup folder.
