...
describe("data directory", () => {
  it("never stores index data next to the source code", () => {
    delete process.env.WA_ARCHIVE_DATA_DIR;
  ...
  it("ignores WA_ARCHIVE_DATA_DIR when it points inside the repo", () => {
    process.env.WA_ARCHIVE_DATA_DIR = path.join(projectRoot(), ".localdata");
    expect(getDataDir()).toBe(defaultDataDir());
  });

  it("still allows an override outside the repo", () => {
    const outside = fs.mkdtempSync(path.join(os.tmpdir(), "wa-data-ok-"));
    process.env.WA_ARCHIVE_DATA_DIR = outside;
